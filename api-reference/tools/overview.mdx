...
- **`description`** — tells the LLM _when_ to call this tool. Be specific. The LLM uses this to decide whether to invoke the tool in a given context.
...
- **`preExecutionMessage`** — an optional message spoken/sent to the user while the tool runs (e.g. _"Let me look that up for you..."_).
