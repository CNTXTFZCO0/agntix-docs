...
In the Agntix dashboard left nav, open **Developer** (under _Integrations_) and click **+ Add Endpoint** in the top-right. A dialog opens with the following fields:
...
- **`session.created`** — fires when the call is _initiated_ (before pickup). Use it to lock the originating task as "in-flight".
