...
Once configured, ask your assistant to list its Agentix tools, or have it run a read-only action such as _"list my Agentix agents."_ A successful response confirms the server URL and key are correct. A `401` means the key is missing or invalid; a `403` means the key lacks the scope for that tool.
