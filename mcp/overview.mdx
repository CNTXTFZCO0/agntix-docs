...
| Best for | Letting an assistant operate Agentix for you | Deterministic, programmatic integrations |
...
```mermaid
flowchart LR
  C["MCP client<br/>(Claude · Cursor · custom)"] -->|tools/call| M["Agentix MCP server"]
  M -->|x-api-key / Bearer| G["Agentix gateway<br/>api.agntix.ai"]
  G --> S["Agents · Chat · Voice · Tools · Telephony"]
```

1. Your MCP client connects to the Agentix MCP server and discovers the available tools (`tools/list`).
