...
3. **Receive a webhook.** Agntix calls your registered endpoint on `session.created` and `session.ended`. Use the echoed `session_variables` to map the call back to your CRM record.
...
```mermaid
flowchart LR
  A[Your backend] -->|1. Trigger call| B[Agntix Voice Agent]
...
Open the agent in the Agntix dashboard. The ID is surfaced in two places on the agent detail screen:
...
3. **Whitelist webhook IPs.** Allow the Agntix outbound webhook IPs through your firewall.
