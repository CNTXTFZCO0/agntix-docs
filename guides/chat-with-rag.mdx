---
title: "Chat with RAG"
description: "Ground an agent's answers in your own documents using an Agentix Knowledge Store."
---
...
2. **Processing** — Agentix chunks the document (~1k tokens per chunk), embeds it (Cohere by default), and writes vectors to Qdrant.
...
Open your agent at `/agents/{id}`. On the right column, switch to the **Knowledge** tab (it's the book icon).

1. Pick your store from the **Select Knowledge Base** dropdown.
2. Adjust retrieval knobs as needed:
   - **Search Type** — vector / hybrid / keyword. *Hybrid* is the safest default.
   - **Top K** — how many chunks to retrieve (range 1–100; default ~6). Higher = more context, higher cost.
   - **Top N** — how many chunks to keep after reranking (must be ≤ Top K).
   - **Agentic RAG** — when on, the agent rewrites the user's question into a better retrieval query before searching. Slower but higher recall on conversational follow-ups.
   - **Rerank** — runs Cohere's reranker over the Top K results. Adds ~150 ms; significantly cuts hallucinations.

<Frame caption="Knowledge tab on the agent detail page. Top K / Top N sliders control retrieval breadth and depth.">
  ![Attach knowledge to agent](/images/guides/chat-with-rag/06-agent-attach-knowledge.png)
</Frame>
...
- The agent's Knowledge tab shows the store selected after saving.
...
|---|---|---|
| Agent ignores the store | Document still **Processing**, or store not saved on the agent | Wait for **Ready**, then re-save the agent |
