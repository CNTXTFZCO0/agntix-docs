...
Once a call ends, Agentix runs an analysis pass over the transcript. Configure what to extract on the agent's **Analytics tab** — open any agent from **Agents** in the left nav, then switch to the **Analytics** tab on the right-hand panel.
...
Define a structured schema and Agentix will pull the values out of the transcript.
